...
public class MegreJoinExecuteTest implements MycatTest {
    ...
    public void testBase() throws Exception {
        ...
        try(Connection mycatConnection = getMySQLConnection(DB_MYCAT);){
            execute(mycatConnection,"/*+ mycat:setBkaJoin{0} */;");
            ...
            sql = "select * from db1.sharding s left join db1.normal e on s.id = e.id inner join db1.global g on s.id = g.id  order by s.id";
            explain= explain(mycatConnection,sql );
            Assert.assertEquals(true,explain.contains("MycatSortMergeJoin"));
            System.out.println();
            ...
            Assert.assertEquals(true,
                    explain.contains("MycatSortMergeJoin"));
            System.out.println();
        }
