...
public class SingleShardingTest extends DrdsTest {
    ...
    public void testSelectGlobal() throws Exception {
        ...
        while (true) {
            Explain explain = parse("select * from db1.global");
            explainColumnSet.add(explain.getColumnInfo());
            explainSet.add(explain.dumpPlan());
            if (explainSet.size() > 1 && (System.currentTimeMillis() > end)) {
                break;
            }
        }
        Assert.assertTrue(
        ...
        Assert.assertEquals("[MycatView(distribution=[Distribution{shardingTables= db1.global ])\n" +
                "c0=SELECT * FROM db1.global, MycatView(distribution=[Distribution{shardingTables= db1.global ])\n" +
                "c1=SELECT * FROM db1.global]",explainSet.toString());
    }
}
