...
import static io.mycat.proxy.packet.MySQLPacketProcessType.*;
...
public interface MySQLPacketResolver extends OkPacket, EOFPacket, ErrorPacket, PreparedOKPacket {
    ...
    PacketSplitter s = new PacketSplitterImpl();

    default boolean readMySQLPayloadFullyNotProxy() throws IOException {
        ...
        if (!multiPacket && !lastIsMultiPacket) {
            ...
            setPayloadStartPos(getStartPos()+4);
            setPayloadEndPos(getEndPos());
            return true;
        }
        if (multiPacket && !lastIsMultiPacket) {
            setWholePacketStartPos(getStartPos());
            setPayloadStartPos(getStartPos());
            return false;
        }
        if (!multiPacket && lastIsMultiPacket) {
            currentProxybuffer().currentBuffer().cutRangeBytesInReading(getStartPos(),getStartPos()+4);
            setEndPos(getEndPos()-4);
        ...
        if (needWholePacket && isPayloadFinished()) {
            ...
            if (remains == 0) {
                ...
                if (packetId != andIncrementPacketId) {
                    throw new MycatExpection("packetId :" + packetId + " andIncrementPacketId:" + andIncrementPacketId);
                }
...
//            }
    ...
    default public void resolvePayloadType(int head, boolean isPacketFinished, boolean parse, MySQLPacket mySQLPacket, int payloadLength) {
...
//    }
    ...
    default void resolvePrepareResponse(MySQLPacket proxyBuf, int head, boolean isPacketFinished) {
        ...
        if (p.getPrepareOkColumnsCount() > 0 && (getState() == ComQueryState.PREPARE_FIELD)) {
            ...
            return;
        } else if (getState() == ComQueryState.PREPARE_FIELD_EOF && head == 0xFE && p.getPrepareOkParametersCount() > 0) {
