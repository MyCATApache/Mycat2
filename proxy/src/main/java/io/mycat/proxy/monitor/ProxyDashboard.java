  ...
  public void collectInfo( ) {
    ...
    for (MycatReactorThread thread : runtime.getMycatReactorThreads()) {
      ...
      for (MycatSession mycat : frontManager.getAllSessions()) {
        ...
        LOGGER.info("byteBuffer:{} in proxyBuffer,writeQueue size:{}",proxyBuffer.currentByteBuffer(),writeQueue);
        ...
        LOGGER.info("charsetName:{} charsetObject :{} characterSetResults:{} ",charsetName,charsetObject,characterSetResults);
        ...
        if (schema!=null){
          LOGGER.info("schema :{}",schema.getSchemaName());
        }
