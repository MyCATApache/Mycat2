...
public enum ConfigFile {
  ...
  ConfigFile(int type, String fileName, Class clazz) {
    this.type = (byte) type;
  ...
  public static ConfigFile getConfigEnum(byte type) {
    ConfigFile[] values = ConfigFile.values();
    int length = values.length;
    for (int i = 0; i < length; i++) {
      ConfigFile value = values[i];
