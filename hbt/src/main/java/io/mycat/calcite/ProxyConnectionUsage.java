...
public class ProxyConnectionUsage {
    ...
    private Future<Map<String, List<SQLKey>>> getConnectionWhenTranscation(XaSqlConnection connection) {
        ...
        getTrxExecutorService().execute(new SequenceTranscationTask(map, connection, promise));
        return promise;
    }
    ...
    private static class SequenceTranscationTask implements Runnable {
        ...
        public SequenceTranscationTask(Map<String, List<SQLKey>> map, XaSqlConnection connection,  PromiseInternal<Map<String, List<SQLKey>>> promise) {
            this.map = map;
        ...
        public void run() {
            try {
                ...
                for (String s : map.keySet()) {
                    future = future
                            .flatMap(unused -> {
                                return (Future) connection.getConnection((s)).mapEmpty();
                            });
                }
