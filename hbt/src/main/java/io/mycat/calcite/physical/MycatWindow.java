...
public class MycatWindow extends Window implements MycatRel {
    ...
    @Override public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
        return new MycatWindow(getCluster(), traitSet, sole(inputs),
    ...
    public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
        ...
        Expression source_ = toEnumerate(builder.append("source", result.block));
        ...
        for (int windowIdx = 0; windowIdx < groups.size(); windowIdx++) {
            Window.Group group = groups.get(windowIdx);
            // Comparator:
            ...
            final RexToLixTranslator.InputGetter inputGetter =
                    new MycatWindow.WindowRelInputGetter(row_, inputPhysType,
        ...
        return block -> new WinAggFrameResultContext() {
            public RexToLixTranslator rowTranslator(Expression rowIndex) {
                Expression row =
                        getRow(rowIndex);
                final RexToLixTranslator.InputGetter inputGetter =
                        new MycatWindow.WindowRelInputGetter(row, inputPhysType,
                                result.physType.getRowType().getFieldCount(),
                                translatedConstants);
    ...
    private Expression translateBound(RexToLixTranslator translator,
                                      ...
                                      Expression max_, Expression rows_, Window.Group group,
                                      boolean lower,
