 ...
 */
public class MycatRexToLixTranslator implements RexVisitor<MycatRexToLixTranslator.Result> {
    ...
    final MycatRexToLixTranslator.InputGetter inputGetter;
    ...
    public MycatRexToLixTranslator(RexProgram program,
    ...
    public static List<Expression> translateProjects(RexProgram program,
        ...
        return new MycatRexToLixTranslator(program, typeFactory, root, inputGetter,
     ...
     */
    public static MycatRexToLixTranslator forAggregation(JavaTypeFactory typeFactory,
                                                         BlockBuilder list, InputGetter inputGetter, SqlConformance conformance) {
        final ParameterExpression root = DataContext.ROOT;
        return new MycatRexToLixTranslator(null, typeFactory, root, inputGetter, list,
                new RexBuilder(typeFactory), conformance, null);
    }

    Expression translate(RexNode expr) {
        final MycatRexImpTable.NullAs nullAs =
                MycatRexImpTable.NullAs.of(isNullable(expr));
        return translate(expr, nullAs);
    }

    Expression translate(RexNode expr, MycatRexImpTable.NullAs nullAs) {
    ...
    Expression translate(RexNode expr, MycatRexImpTable.NullAs nullAs,
        ...
        if (MycatRexImpTable.NullAs.NOT_POSSIBLE == nullAs
            ...
            case DATE:
                switch (sourceType.getSqlTypeName()) {
                    ...
                    case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
                        convert = MycatRexImpTable.optimize2(
                                operand,
                                Expressions.call(
            ...
            case INTERVAL_SECOND:
                switch (sourceType.getSqlTypeName().getFamily()) {
                    case NUMERIC:
                        ...
                        convert = MycatRexImpTable.multiplyDivide(convert, multiplier, divider);
                }
        }
    ...
    public static Expression translateLiteral(
            ...
            RelDataType type,
            JavaTypeFactory typeFactory,
            MycatRexImpTable.NullAs nullAs) {
        if (literal.isNull()) {
            switch (nullAs) {
                case TRUE:
                case IS_NULL:
                    return MycatRexImpTable.TRUE_EXPR;
                case FALSE:
                case IS_NOT_NULL:
                    return MycatRexImpTable.FALSE_EXPR;
                case NOT_POSSIBLE:
                    throw new ControlFlowException();
                case NULL:
                default:
                    return MycatRexImpTable.NULL_EXPR;
            }
        } else {
            switch (nullAs) {
                case IS_NOT_NULL:
    ...
    public static Expression translateCondition(RexProgram program,
        ...
        MycatRexToLixTranslator translator =
                new MycatRexToLixTranslator(program, typeFactory, root, inputGetter, list,
                        new RexBuilder(typeFactory), conformance, null);
        translator = translator.setCorrelates(correlates);
        return translator.translate(
                program.getCondition(),
                MycatRexImpTable.NullAs.FALSE);
    }
    ...
    public MycatRexToLixTranslator setBlock(BlockBuilder block) {
        if (block == list) {
            return this;
        }
        return new MycatRexToLixTranslator(program, typeFactory, root, inputGetter,
    ...
    public MycatRexToLixTranslator setCorrelates(
            Function1<String, InputGetter> correlates) {
        if (this.correlates == correlates) {
            return this;
        }
        return new MycatRexToLixTranslator(program, typeFactory, root, inputGetter, list,
            ...
            case NUMERIC:
                switch (sourceType.getSqlTypeName()) {
                    ...
                    case INTERVAL_SECOND:
                        ...
                        return MycatRexImpTable.multiplyDivide(operand, multiplier, divider);
                }
        }
        return operand;
    }
    ...
    public Result visitLiteral(RexLiteral literal) {
        ...
        final Expression valueExpression = literal.isNull()
                ...
                typeFactory, MycatRexImpTable.NullAs.NOT_POSSIBLE);
        final ParameterExpression valueVariable =
        ...
        final Expression isNullExpression =
                literal.isNull() ? MycatRexImpTable.TRUE_EXPR : MycatRexImpTable.FALSE_EXPR;
    ...
    public Result visitCall(RexCall call) {
        ...
        final MycatRexImpTable.RexCallImplementor implementor =
                MycatRexImpTable.INSTANCE.get(operator);
        if (implementor == null) {
    ...
    private static Result implementCallOperand(final RexNode operand,
                                               final Type storageType, final MycatRexToLixTranslator translator) {
     ...
     * </pre></blockquote>
     */
    private void implementRecursively(final MycatRexToLixTranslator currentTranslator,
        ...
        final MycatRexToLixTranslator ifTrueTranslator =
        ...
        final BlockBuilder ifFalseBlockBuilder =
                new BlockBuilder(true, currentBlockBuilder);
        final MycatRexToLixTranslator ifFalseTranslator =
    ...
    public Result visitFieldAccess(RexFieldAccess fieldAccess) {
        ...
        switch (target.getKind()) {
            case CORREL_VARIABLE:
                ...
                final MycatRexToLixTranslator.InputGetter getter =
    ...
    Expression checkNull(Expression expr) {
        ...
        return Expressions.equal(expr, MycatRexImpTable.NULL_EXPR);
    }
    ...
    Expression checkNotNull(Expression expr) {
        ...
        return Expressions.notEqual(expr, MycatRexImpTable.NULL_EXPR);
    }
