  ...
  public static boolean checkInputForCollationAndLimit(RelMetadataQuery mq,
    ...
    if (rowCount != null && fetch != null) {
      ...
      if (offset instanceof RexDynamicParam){
          int index = ((RexDynamicParam) offset).getIndex();
          List<Object> params = ParamHolder.CURRENT_THREAD_LOCAL.get().getParams();
          if (params.size()>index){
            Object o =params.get(index);
            offset = MycatCalciteSupport.RexBuilder.makeExactLiteral(new BigDecimal(o.toString()));
          }else {
            offset = MycatCalciteSupport.RexBuilder.makeExactLiteral(BigDecimal.ZERO);
          }
      }
      if (fetch instanceof RexDynamicParam){
        ...
        if (params.size()>index){
          Object o =params.get(index);
          fetch = MycatCalciteSupport.RexBuilder.makeExactLiteral(new BigDecimal(o.toString()));
        }else {
          fetch = MycatCalciteSupport.RexBuilder.makeExactLiteral(BigDecimal.valueOf(1000));
        }
      }
