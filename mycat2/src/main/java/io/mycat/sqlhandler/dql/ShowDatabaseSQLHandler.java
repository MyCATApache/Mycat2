...
public class ShowDatabaseSQLHandler extends AbstractSQLHandler<SQLShowDatabasesStatement> {
    ...
    @Override
    protected int onExecute(SQLRequest<SQLShowDatabasesStatement> request, Receiver response, MycatSession session) {
        //直接调用已实现好的
        request.getContext().utilityStatementHandler().handleSQLShowDatabasesStatement(request.getAst(),response);
        return CODE_200;
    }
}
