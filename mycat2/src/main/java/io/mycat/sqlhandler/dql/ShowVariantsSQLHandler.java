...
public class ShowVariantsSQLHandler extends AbstractSQLHandler<MySqlShowVariantsStatement> {
    ...
    @Override
    protected int onExecute(SQLRequest<MySqlShowVariantsStatement> request, Receiver response, MycatSession session) {
        //直接调用已实现好的
        request.getContext().showStatementHandler().handleMySqlShowVariants(request.getAst(), response);
        return CODE_200;
    }
}
