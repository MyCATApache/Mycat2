...
public class VertxExecuter {
        ...
        Future<RowObservable> observableFuture = sqlConnectionFuture
                .flatMap(connection -> connection.prepare(sql)).compose(preparedStatement -> {
                    ...
                    RowObservable observable = new RowObservable() {
                        ...
                        @Override
                        protected void subscribeActual(@NonNull io.reactivex.rxjava3.core.Observer<? super Object[]> observer) {
    ...
    private Collector<Row, io.reactivex.rxjava3.core.Observer, io.reactivex.rxjava3.core.Observer> getCollector(ProcessMonitor monitor, io.reactivex.rxjava3.core.Observer<Object[]> observer) {
        return new Collector<Row, io.reactivex.rxjava3.core.Observer, io.reactivex.rxjava3.core.Observer>() {
            @Override
            public Supplier<io.reactivex.rxjava3.core.Observer> supplier() {
                return () -> observer;
            }

            @Override
            public BiConsumer<io.reactivex.rxjava3.core.Observer, Row> accumulator() {
                return (aVoid, row) -> aVoid.onNext(row);
            }

            @Override
            public BinaryOperator<io.reactivex.rxjava3.core.Observer> combiner() {
