...
import io.ordinate.engine.vector.BinaryExpressionContext;
...
public class CalcPlan implements PhysicalPlan {
    private static final Logger LOGGER = LoggerFactory.getLogger(CalcPlan.class);
    final PhysicalPlan inputPlan;
    final Function condition;
    private List<VectorExpression> exprs;
    final org.apache.arrow.vector.types.pojo.Schema schema;

    public CalcPlan(PhysicalPlan input, Function condition, List<VectorExpression> exprs, org.apache.arrow.vector.types.pojo.Schema schema) {
    ...
    public Observable<VectorSchemaRoot> execute(RootContext rootContext) {
        return inputPlan.execute(rootContext).map(input -> {
            ...
            bitVector.setValueCount(rowCount);
            VectorBatchRecord recordVectorSchemaRoot = new VectorBatchRecord(input);
            try {
                for (int i = 0; i < rowCount; i++) {
                    recordVectorSchemaRoot.setPosition(i);
                    int value = condition.getInt(recordVectorSchemaRoot);
                    if (value>0){
