...
public class MulDoubleFunctionFactory implements FunctionFactory {
    ...
    public Function newInstance(List<Function> args, EngineConfiguration configuration) {
        return new MulDoubleFunc((ScalarFunction) args.get(0), (ScalarFunction) args.get(1));
    }

    private static class MulDoubleFunc extends DoubleFunction implements BinaryArgFunction {
        final ScalarFunction left;
        final ScalarFunction right;
        boolean isNull;

        public MulDoubleFunc(ScalarFunction left, ScalarFunction right) {
            super();
            this.left = left;
            this.right = right;
        }
