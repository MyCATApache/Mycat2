...
public class QuarterFunctionFactory implements FunctionFactory {

    @Override
    public String getSignature() {
        return "quarter(date):int";
    }
    ...
    private static final class Func extends IntFunction {
        ...
        public int getInt(Record rec) {
            ...
            long a = one.getDate(rec);
            isNull = one.isNull(rec);
            if (isNull){
                return 0;
            }
            return QuarterFunction.quarter(new Date(a).toLocalDate());
        }
    }
}
