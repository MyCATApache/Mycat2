...
public class SubDoubleFunctionFactory implements FunctionFactory {
    ...
    public Function newInstance(List<Function> args, EngineConfiguration configuration) {
        return new SubDoubleFunc((ScalarFunction)args.get(0), (ScalarFunction)args.get(1));
    }

    private static class SubDoubleFunc extends DoubleFunction implements BinaryArgFunction {
        final ScalarFunction left;
        final ScalarFunction right;
        boolean isNull;
        public SubDoubleFunc(ScalarFunction left, ScalarFunction right) {
            super();
            this.left = left;
            this.right = right;
        }
