...
//		return (short) ((0xff & bytes[0]) | (0xff00 & (bytes[1] << 8)));
    ...
    public static byte[] getBytesFromDate(LocalDate date) {
    ...
    public static byte[] getBytesFromTimestamp(LocalDateTime date) {
        if (date == null) {
            return null;
        }
        int year = date.getYear();
        ...
        int microSecond = (int) TimeUnit.NANOSECONDS.toMicros(date.getNano());
        byte[] bytes = null;
        byte[] tmp = null;
    ...
    public static byte[] getBytesFromTime(LocalTime date) {
        if (date == null) return null;
        long seconds = date.getSecond();
    ...
    public static byte[] getBytesFromTimeString(String date) {
        date = date.trim();
        int i = date.indexOf(' ');
        int day = 0;
        if (i != -1) {
    ...
    public static byte[] getBytesFromDuration(Duration date) {
        ...
        if (day == 0 && hour == 0 && minute == 0
            ...
            bytes = new byte[1 + 8];
            bytes[0] = (byte) 8;
            bytes[1] = (byte) (date.isNegative() ? 1 : 0); // is_negative (1) -- (1 if minus, 0 for plus)
            tmp = getBytes(day);
            bytes[2] = tmp[0];
            bytes[3] = tmp[1];
            bytes[4] = tmp[2];
