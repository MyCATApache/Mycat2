 ...
 * <p>
 * 2017/2/24 表名规范遵循 ： https://dev.mysql.com/doc/refman/5.7/en/identifiers.html 暂时不支持字符型表名
 * https://github.com/mysql/mysql-workbench/blob/1972008552c725c4176fb83a35734cf2c1f6158c/library/parsers/grammars/MySQL.g
 * 2017/2/25 SQLBenchmark.DruidTest          thrpt   10   225009.114 ±  4904.006  ops/s
 * SQLBenchmark.NewSqQLParserTest  thrpt   10  3431639.918 ± 54679.339  ops/s
 * SQLBenchmark.SQLParserTest      thrpt   10  1297501.144 ± 56213.315  ops/s 2017/2/26 以 --
 * 开头的注释直接跳过 sql内字符串只保留字符串标识，第二阶段需要的话再根据pos和size进行解析 sql内数值只保留数值标识，第二阶段需要的话再根据pos和size进行解析
 ...
 * 计划用建立512大小的关键字哈希索引数组，最长碰撞为5，数组有效数量是379（ & 0x3fe >> 1） 2017/3/8 还可以进行如下优化： 1. 部分变量可以替换成常量（是否需要预编译）
 * 2. 使用堆外unsafe数组 3. SQLContext还需要优化成映射到hashArray的模式，也可以考虑只用一个数组，同时存储hashArray、charType、token和解析结果（估计也没人看得懂了）
 * 2017/3/31 NewSQLContext部分还需要进行如下优化 1. 支持cache hint 例如 /*!mycat:cache-time=xxx auto-refresh=true
 * access-count=5000... 2. 支持每一句SQL都有独立的注解 3. 支持包含注解的语句提取原始sql串
 */
    ...
    while (pos < arrayCount) {
      switch (hashArray.getIntHash(pos)) {
        ...
        case IntTokenHash.SELECT:
          if (pos > 0 && hashArray.getType(pos - 1) == Tokenizer.LEFT_PARENTHESES) {
            context.setHasSubQuery(true);
          }
...
//                    pos = DCLSQLParser.pickRevoke(++pos, arrayCount, context, hashArray, sql);
        ...
        default:
          ...
          switch (type) {
            ...
            case Tokenizer.LESS_EQUAL_GREATER:
            case Tokenizer.LESS_GREATER:
//            case Tokenizer.LESS_LESS:
            case Tokenizer.GREATER:
            case Tokenizer.GREATER_EQUAL:
//            case Tokenizer.GREATER_GREATER:
              context.setHasCompare(true);
            default:
