...
public class Tokenizer {
    ...
    int parseToken(ByteArrayView sql, int pos, final int sqlLength, byte c) {
        ...
        while (++pos < sqlLength && (((cType = charType[(c = sql.getByte(pos))<<1]) == 2) || cType == 1) ) {
            ...
            switch (cType) {
                ...
                case DIVISION: {// /
                    ...
                    if (next == '*') {//  /*
                        ...
                        //处理新版 mycat 注解
                        if (next == ' ') {
                            if ((sql.getByte(++pos) & 0xDF) == 'M' && (sql.getByte(++pos) & 0xDF) == 'Y' && (sql.getByte(++pos) & 0xDF) == 'C' && (sql.getByte(++pos) & 0xDF) == 'A' && (sql.getByte(++pos) & 0xDF) == 'T'
                                    && sql.getByte(++pos) == ':') {
