...
public class HashFunctionTest extends AutoFunctionFactoryTest{
    ...
    public void testHashIdHashId() {
        ...
        List<Partition> calculate = shardingFuntion.calculate(Collections.singletonMap("id", (new RangeVariable("id", Type.NUMBER, RangeVariableType.EQUAL, 15))));
    ...
    public void testHashIdHashUserId() {
        ...
        ShardingTable tableHandler = (ShardingTable) metadataManager.getTable("db1", "sharding");
        CustomRuleFunction shardingFuntion = tableHandler.getShardingFuntion();
        ImmutableMap<String, RangeVariable> map = ImmutableMap.of("id", new RangeVariable("id", Type.NUMBER, RangeVariableType.EQUAL, 15),
                "user_id", (new RangeVariable("user_id", Type.STRING, RangeVariableType.EQUAL, 2))
        );
        ...
        List<Partition> firstColumn = shardingFuntion.calculate(
                ImmutableMap.of("id",(new RangeVariable("id", Type.NUMBER, RangeVariableType.EQUAL, 15))));

        Assert.assertTrue(firstColumn.containsAll(insertRoute));

        List<Partition> secondColumn = shardingFuntion.calculate(
                ImmutableMap.of("user_id",(new RangeVariable("user_id", Type.STRING, RangeVariableType.EQUAL, 2))));
