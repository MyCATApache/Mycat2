            ...
            try {
               ...
               boolean allIncrentmentSucess=true;
                ...
                for (String dataHostName : dataHosts) {
                    ...
                    Type type = new TypeToken<ArrayList<MigrateTask>>(){}.getType();

                    List<MigrateTask> migrateTaskList= new Gson().fromJson(new String(ZKUtils.getConnection().getData().forPath(taskPath+"/"+dataHostName),"UTF-8") ,type);
                    for (MigrateTask migrateTask : migrateTaskList) {
                        ...
                        if (ZKUtils.getConnection().checkExists().forPath(zkPath) != null) {
                            TaskStatus taskStatus = new Gson().fromJson(
