            ...
            try {
                ...
                if (!dataNodeList.isEmpty()) {
                    if ((!Strings.isNullOrEmpty(text)) && text.startsWith("{")) {
                        TaskNode taskNode = new Gson().fromJson(text, TaskNode.class);
                        if (taskNode.getStatus() == 0) {
                            ...
                            for (String s : dataNodeList) {
                                ...
                                if (dataNodes.contains(s)) {
                                    ...
                                    TypeToken<ArrayList<MigrateTask>> arrayListTypeToken = new TypeToken<ArrayList<MigrateTask>>(){};
                                    List<MigrateTask> migrateTaskList = new Gson().fromJson(data, arrayListTypeToken.getType());
                            ...
                            ZKUtils.getConnection().setData().forPath(event.getData().getPath(), JSONUtils.toJSONString(taskNode).getBytes());
