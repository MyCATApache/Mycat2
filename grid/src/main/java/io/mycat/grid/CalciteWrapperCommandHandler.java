...
public class CalciteWrapperCommandHandler extends AbstractCommandHandler {

  final static MycatLogger LOGGER = MycatLoggerFactory
      .getLogger(CalciteWrapperCommandHandler.class);
  final static ExecutorService service = Executors
      .newCachedThreadPool(r -> new ReactorEnvThread(r) {
      });

  final CalciteCommandHandler handler;

  public CalciteWrapperCommandHandler() {
    handler = new CalciteCommandHandler();
