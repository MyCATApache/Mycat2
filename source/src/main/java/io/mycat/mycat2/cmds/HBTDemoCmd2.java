...
public class HBTDemoCmd2 implements MySQLCommand {
	...
	public boolean procssSQL(MycatSession session) throws IOException {
		...
		// where topic_id in "
		// +
		// "('1DDA8F5EC0A82CB200000817177360','1DDA8E99C0A82CB200000543452550','1DDA8E9EC0A82CB200000549901449',
		// '1DDA8EB3C0A82CB200000573846014');";
		String sql2 = "select title,seq, topic_id, question_id from e_question ";
		// FetchIntoRowStream fetchIntoRowStream = new FetchIntoRowStream(new
		// SqlMeta(sql,"a"),new RowMeta("tableA"));
		HBTEngine engine = new HBTEngine();
		String[] fieldNameList = { "topic_id", "question_type", "update_time", "seq", "question_id" };
		int[] fieldTypeList = { Fields.FIELD_TYPE_STRING, Fields.FIELD_TYPE_STRING, Fields.FIELD_TYPE_STRING,
				Fields.FIELD_TYPE_STRING, Fields.FIELD_TYPE_STRING, };
		ResultSetMeta resultSetMeta = new ResultSetMeta(Arrays.asList(fieldNameList), fieldTypeList);

		// String[] fieldNameList3 =
		// {"e_topic.topic_id","e_topic.question_type","e_question.seq","e_question.question_id","item_id"};
		// int[] fieldTypeList3 = {Fields.FIELD_TYPE_STRING,
		// Fields.FIELD_TYPE_STRING,
		// Fields.FIELD_TYPE_STRING, Fields.FIELD_TYPE_STRING,
		// Fields.FIELD_TYPE_STRING,};
		//
		// String sql3 = "select item_id, question_id from e_item";
		// ResultSetMeta resultSetMeta2 = new
		// ResultSetMeta(Arrays.asList(fieldNameList3),
		// fieldTypeList3);

		String[] groupSetMetaList = { "count", "topic_id", "question_type", "update_time", "question_id" };
		int[] groupSetMetaTypeList = { Fields.FIELD_TYPE_INT24, Fields.FIELD_TYPE_STRING, Fields.FIELD_TYPE_STRING,
				Fields.FIELD_TYPE_STRING, Fields.FIELD_TYPE_STRING, };

		ResultSetMeta groupResultSetMeta = new ResultSetMeta(Arrays.asList(groupSetMetaList), groupSetMetaTypeList);
		...
		engine.streamOf(session, new SqlMeta(sql, "et"), new RowMeta("e_topic", "et"))
				.join(session, new SqlMeta(sql2, "eq"), new RowMeta("e_question", "eq"),
						new JoinMeta("e_topic.topic_id", "e_question.topic_id", HBTEngine.MEM, 100), resultSetMeta,
						(aRow, bRow, result) -> {
							result.addAll(aRow);
							result.add(bRow.get(1));
							result.add(bRow.get(2));
						})
				.group(groupPairKeyMeta, groupResultSetMeta,
						Arrays.asList(new CountFunction(), new OutFunction(0, 1, 2, 3)))
				.order(new OrderMeta(Arrays.asList("update_time", "count", "topic_id"),
						Arrays.asList(SortOrder.DESCENDING, SortOrder.ASCENDING, SortOrder.ASCENDING)))
				.limit(11).out(session);
		return true;
		// }
		// return false;
	}
