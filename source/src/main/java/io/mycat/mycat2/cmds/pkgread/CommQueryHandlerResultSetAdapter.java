...
public class CommQueryHandlerResultSetAdapter implements CommandHandler {
	...
	public static final CommQueryHandlerResultSetAdapter INSTANCE = new CommQueryHandlerResultSetAdapter();
			...
			case Full:
				...
				if (session.curMSQLPackgInf.pkgType == MySQLPacket.EOF_PACKET) {
					...
					// 如果当前列列结束，则进行结束标识验证
						...
						session.getMycatSession().commandHandler = CommQueryHandlerAdapter.INSTANCE;
