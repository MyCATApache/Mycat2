...
//					// 首先检查当前列标识结果
//					if (!session.getAttrMap().containsKey(SessionKey.SESSION_KEY_COLUMN_OVER)) {
//						session.getAttrMap().put(SessionKey.SESSION_KEY_COLUMN_OVER, true);
//					}
//					// 如果当前列列结束，则进行结束标识验证
//					else {
//						// 进行标识重置
//						session.getAttrMap().remove(SessionKey.SESSION_KEY_COLUMN_OVER);
//						isFinish = true;
//
...
//		if (!isFinish) {
//			// 标识当前传输未结束
//			//mycatSession.getAttrMap().put(SessionKey.SESSION_KEY_TRANSFER_OVER_FLAG.getKey(), true);
//		} else {
//			// 结束移除标识
//			mycatSession.getAttrMap().remove(SessionKey.SESSION_KEY_TRANSFER_OVER_FLAG.getKey());
//		}
//
