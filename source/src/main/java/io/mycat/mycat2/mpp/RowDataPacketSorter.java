...
public class RowDataPacketSorter {
    ...
    private void compare(int byColumnIndex) {
        ...
        byte[] left = array[p1].fieldValues.get(this.orderCols[byColumnIndex].columnMeta.colIndex);
        byte[] right = array[p2].fieldValues.get(this.orderCols[byColumnIndex].columnMeta.colIndex);
    ...
    public static final int compareObject(byte[] left,byte[] right, OrderCol orderCol) {
        int colType = orderCol.getColumnMeta().getColType();
        switch (colType) {
