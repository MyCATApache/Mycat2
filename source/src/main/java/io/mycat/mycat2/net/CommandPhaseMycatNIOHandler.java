 ...
 *
 */
public class CommandPhaseMycatNIOHandler implements NIOHandler<MycatSession> {
	public static final CommandPhaseMycatNIOHandler INSTANCE = new CommandPhaseMycatNIOHandler();
	private static Logger logger = LoggerFactory.getLogger(CommandPhaseMycatNIOHandler.class);
	...
	public void onSocketRead(final MycatSession session) throws IOException {
		...
		if (!session.getAttrMap().containsKey(SessionKey.PKG_READ_FLAG) && readed == false) {
			return;
		}
