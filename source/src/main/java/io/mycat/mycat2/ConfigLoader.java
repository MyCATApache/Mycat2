...
public class ConfigLoader {
    ...
    public static List<SchemaBean> loadSheamBeans(String schemaBeanuri) {
        ...
        try {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                SchemaBean sBean =
                        new SchemaBean(name, dnBean, ("true".equalsIgnoreCase(schemaType)),
                                tableLst);
        ...
        try {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                List<MySQLBean> allMysqls = mysqlNodes.stream().map(mysqlNode -> {
                    ...
                    String host = getAttribute(attrs, "hostname", null);
    ...
    private static List<Node> getChildNodes(Node theNode, String childElName) {
        ...
        for (int j = 0; j < childs.getLength(); j++) {
            if (childs.item(j).getNodeType() == Document.ELEMENT_NODE && childs.item(j)
                    .getNodeName().equals(childElName)) {
